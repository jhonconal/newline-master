    ...
    ui(new Ui::UploadWidget)
{
    ...
    isExist = true;
    initValue();
//    IconHelper::Instance()->SetIcon(ui->statusLabel,QChar(0xf062),qRound(30/g_fontPixelRatio));//upload
...
void UploadWidget::SoltSendFileToAndroidFailed()
{
    ...
    {//仅仅接受一次发送失败信号
        ...
        hint->setGeometry((this->pos().x()+this->width()/2)- (hint->width())+hint->width()/3+100,
    ...
    m_failedClose =true;
    this->close();
//    IconHelper::Instance()->SetIcon(ui->statusLabel,QChar(0xf00d),30/g_fontPixelRatio);//error
//    update();
...
void UploadWidget::drawTextValue(QPainter *painter)
{
    ...
    {
        font.setPixelSize(qRound(56/g_fontPixelRatio));
    }
    else
    {
...
//        IconHelper::Instance()->SetIcon(ui->statusLabel,QChar(0xf00c),qRound(30/g_fontPixelRatio));//成功
        ui->statusLabel->setStyleSheet("background-color: rgb(56, 166, 223);"
                                                                  "color: rgb(255, 255, 255);"
