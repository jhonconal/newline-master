...
//                HHT_LOG(EN_INFO, "huang, datalen:%d, EndPos:%d, startPos:%d", nDataLen, m_nEndPos, m_nStartPos);
                ...
                if (nEndPos < COM_BUF_LEN)
                {
                    ...
                    else
                    {
                        m_nStartPos += nReadLen;
                        m_nRealBufLen -= nReadLen;
                        HHT_LOG(EN_ERR, "========>return -3");
                        return -3;
                    }
                }
                else
                {
                    HHT_LOG(EN_ERR, "========>return -4");
                    return -4;
                }
            }
            else                           //buffer头大于尾
...
INT32 COMDEV::WriteCom(UINT8 *pBuf, const INT32 nLen)
{
    ...
    {
        HHT_LOG(EN_ERR, "WriteCom null error return");
        return RET_INVALID;
    }
...
//    }
    ...
    {
        HHT_LOG(EN_ERR, "WriteFile false return .");
        return RET_INVALID;
    }
